...
func (m *MockmessagesRepository) GetClientChatMessages(ctx context.Context, clientID types.UserID, pageSize int, cursor *messages.Cursor) ([]messages.Message, *messages.Cursor, error) {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "GetClientChatMessages", ctx, clientID, pageSize, cursor)
	ret0, _ := ret[0].([]messages.Message)
	ret1, _ := ret[1].(*messages.Cursor)
	ret2, _ := ret[2].(error)
